document.addEventListener("DOMContentLoaded", () => {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            if (!storedUser || storedUser.rol !== "FABRICA") {
    ...
    function attachEventListeners() {
        ...
        document.querySelectorAll(".edit_btn").forEach(button => {
            button.addEventListener("click", function () {
                showEditProduct(this.dataset.id);
            });
        });
            ...
            if (cantidad > 0) {
                ...
                for (let i = 0; i < cantidad; i++) {
                    etiquetas.push({
                        ...
                        leather: detailLeather.textContent,
                        price: detailPrice.textContent,
                        price2: (parseFloat(detailPrice.textContent) * 1.06).toFixed(2),
                        cod: codigo
                    });
                }
            }
    ...
    //Verificar valores antes de editar
    editDetailTxt34.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt35.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt36.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt37.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt38.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt39.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt40.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt41.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt42.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt43.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    editDetailTxt44.addEventListener("change", function({target}){
        if(target.value <=0){
            target.value = "";
        }
    });
    ...
    btnConfirmSend.addEventListener("click", function(){
        if(localSend.value == ""){
            showAlertSend("Debe elegir un local para generar el envío", "E");
            return;
        };
        if(Object.keys(productSendList).length == 0){
            showAlertSend("Debe escanear al menos un producto", "E");
            return;
        };
        showAlertSend("CARGANDO...", "L");
        const now = new Date();
        ...
        db.collection("ENVIOS").add({
            ...
            showAlertSend(error.message, "E");
        }).finally(() => showAlertSend("", "C"));
    });
    ...
    btnSaveClient.addEventListener("click", function() {
        if(txtIdentificationClientAdd.value == "" || txtNameClientAdd.value == "" || 
            ...
            showAlertAddClient("Debe llenar los campos obligatorios", "E");
        }else{
            db.collection("CLIENTES").doc(txtIdentificationClientAdd.value).set({
                ...
                showAlertAddClient(error.message, "E");
            })
        }
    });
    ...
    //ALERTAS MODAL EDITAR
    const alertMessageEdit = document.getElementById("alert_message_edit");

    function showAlertEdit(message, type) {
        alertMessageEdit.innerText = message;
        alertMessageEdit.style.display = "block";
        if(type == "S"){
            alertMessageEdit.style.background = "#5ccb5f";
            setTimeout(() => {
                alertMessageEdit.style.display = "none";
            }, 3000);
        }else if(type == "E"){
            alertMessageEdit.style.background = "#ef9a9a";
            setTimeout(() => {
                alertMessageEdit.style.display = "none";
            }, 3000);
        }else if(type == "L"){
            alertMessageEdit.style.background = "#e0e0e0";
        }else if(type == "C"){
            alertMessageEdit.style.display = "none";
        }
    }

    //ALERTAS MODAL ENVIAR
    const alertMessageSend = document.getElementById("alert_message_send");

    function showAlertSend(message, type) {
        alertMessageSend.innerText = message;
        alertMessageSend.style.display = "block";
        if(type == "S"){
            alertMessageSend.style.background = "#5ccb5f";
            setTimeout(() => {
                alertMessageSend.style.display = "none";
            }, 3000);
        }else if(type == "E"){
            alertMessageSend.style.background = "#ef9a9a";
            setTimeout(() => {
                alertMessageSend.style.display = "none";
            }, 3000);
        }else if(type == "L"){
            alertMessageSend.style.background = "#e0e0e0";
        }else if(type == "C"){
            alertMessageSend.style.display = "none";
        }
    }

    //ALERTAS MODAL AÑADIR CLIENTE
    const alertAddClient = document.getElementById("alertAddClient");

    function showAlertAddClient(message, type) {
        alertAddClient.innerText = message;
        alertAddClient.style.display = "block";
        if(type == "S"){
            alertAddClient.style.background = "#5ccb5f";
            setTimeout(() => {
                alertAddClient.style.display = "none";
            }, 3000);
        }else if(type == "E"){
            alertAddClient.style.background = "#ef9a9a";
            setTimeout(() => {
                alertAddClient.style.display = "none";
            }, 3000);
        }else if(type == "L"){
            alertAddClient.style.background = "#e0e0e0";
        }else if(type == "C"){
            alertAddClient.style.display = "none";
        }
    }
