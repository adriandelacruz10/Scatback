document.addEventListener("DOMContentLoaded", () => {
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            if (!storedUser || storedUser.rol !== "CUENCA") {
    ...
    txtScanSell.addEventListener("keydown", function(event) {
        if (event.key === "Enter") {
            ...
            db.collection("CUENCA").doc(code.toString()).get()
            .then((doc) => {
                if(doc.exists){
    ...
    btnAddProduct.addEventListener("click", async function () {
        if(txtNameSell.textContent == "" || txtNameClient.textContent == ""){
            ...
            try{
                const product = (await db.collection("CUENCA").doc(code.toString()).get()).data();
    ...
    async function buscarProducto(cod){
        const doc = await db.collection('CUENCA').doc(cod).get();
        if(!doc.exists){
    ...
    async function actualizarStockCambio(cod, tal, can){
        ...
        await db.collection("CUENCA").doc(cod).update({
            [talla]: act
        });
    }
    ...
    btnConfirmarVenta.addEventListener('click', async function() {
        ...
        if(txtComprobanteVenta.value == ""){
            ...
            try {
                ...
                for(const p of listProductsSell){
                    const product = (await db.collection('CUENCA').doc(p.cod.toString()).get()).data();
                    if(product.temporal == 'X'){
                        ...
                        const talla = `t${p.tal}`;
                        await db.collection('CUENCA').doc(p.cod.toString()).update({
                            [talla]: act
                        });
                    }
                }
    ...
    async function cargarEnvios() {
        try{
            ...
            const datos = await db.collection("ENVIOS").where("tratado", "==", "").where("destino", "==", "Cuenca").get();
            if(datos.empty){
                divRecibir.style.display = "none";
    ...
    async function actualizarStock(id, ind, cod, tal){
        try{
            ...
            if(doc.exists){
                ...
                await db.collection("CUENCA").doc(cod).set({
                    nombre: pro.nombre,
                    cuero: pro.cuero,
    ...
    function showProductDetails(productId) {
        db.collection("CUENCA").doc(productId).get().then((doc) => {
            if (doc.exists) {
                const product = doc.data();
    ...
    txtScan.addEventListener("keydown", async function(event) {
        if (event.key === "Enter") {
            ...
            try{
                const product = await obtenerStock("CUENCA", code);
    ...
    async function reporteDiario(){
        ...
        const ventas = await obtenerVentas('DI', 'Cuenca', txtFechaRep.value);
        const abonos = await obtenerAbonos('DI', 'Cuenca', txtFechaRep.value);
    ...
    async function reporteMensual(){
        ...
        const ventas = await obtenerVentas('ME', 'Cuenca', txtFechaRep.value);
        const abonos = await obtenerAbonos('ME', 'Cuenca', txtFechaRep.value);
...
btnConfirmarCambio.addEventListener('click', async function(){
    ...
    const batch = db.batch();
        ...
        const destinoRef = db.collection('CUENCA').doc(doc.id);
        batch.set(destinoRef, datos);
    });
    await batch.commit();
    console.log(`Se copiaron ${productosSnap.size} documentos a CUENCA.`);
})
ACTUALIZAR TODO EL CAMPO DE UNA BASE
async function obtenerVentasAnual(){
  ...
  for (const doc of snapshot.docs) {
    await doc.ref.update({ anio: "2025" });
    contador++;
    console.log(`Actualizado: ${doc.id}`);
  }

  console.log(`✅ Campo 'anio' agregado a ${contador} documentos`);
}
*/